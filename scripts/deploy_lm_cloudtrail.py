...
def find_source_cloudtrail(token, endpoint, target_ct_name, target_cid):
	API_ENDPOINT = endpoint + target_cid + "/sources?type=s3aws&name=" + target_ct_name
	REQUEST = requests.get(API_ENDPOINT, headers=HEADERS, auth=(token,''))
...
if __name__ == '__main__':
	...
	find_parser = subparsers.add_parser("FIND", help="Find CloudTrail collection")
	...
	add_parser.add_argument("--force", required=False, help="Override check and force create CloudTrail Log source", default=False, action='store_true')
	...
	del_parser.add_argument("--uid", required=False, help="Cloudtrail source ID that you wish to delete", default="")
	del_parser.add_argument("--ct", required=False, help="Name of Cloudtrail log source to search and delete")
	...
	#Parser argument for Find scope
	find_parser.add_argument("--key", required=True, help="User Key for Alert Logic Log Manager API Authentication")
	find_parser.add_argument("--cid", required=True, help="Alert Logic Customer CID as target")
	find_parser.add_argument("--ct", required=True, help="Name of Cloudtrail log source to search")
	find_parser.add_argument("--dc", required=True, help="Alert Logic Data center assignment, i.e. defender-us-denver, defender-us-ashburn or defender-uk-newport")
	...
	if args.mode == "ADD":
		...
		if ALERT_LOGIC_LM != False:
			print ("### Check for existing Cloud Trail Link ###")
			CT_FIND_RESULTS = find_source_cloudtrail(APIKEY, ALERT_LOGIC_LM, TARGET_CT_NAME, TARGET_CID)
			if CT_FIND_RESULTS != "n/a" and CT_FIND_RESULTS["total_count"] > 0:
				print ("CloudTrail Log Source with the same name already exist:\n" +str(json.dumps(CT_FIND_RESULTS, indent=2)) )
				if args.force == False:
					print ("Skip creating new CloudTrail, use --force to override\n")
					EXIT_CODE=0
					sys.exit(EXIT_CODE)
				else:
					print ("Override enabled, will attempt to create CloudTrail Log Source with similar name\n")
			else:
				print ("No Cloudtrail Log Source found with similar name, script resume\n")
		...
		if TARGET_CT_ID != "":
			print ("Delete CloudTrail Log Source with ID : " + TARGET_CT_ID)
			CT_RESULT = get_source_cloudtrail(APIKEY, ALERT_LOGIC_LM, TARGET_CT_ID, TARGET_CID)
		else:
			print ("Delete CloudTrail Log Source with Name : " + TARGET_CT_NAME)
			CT_FIND_RESULTS = find_source_cloudtrail(APIKEY, ALERT_LOGIC_LM, TARGET_CT_NAME, TARGET_CID)
			if CT_FIND_RESULTS != "n/a" and CT_FIND_RESULTS["total_count"] > 0:
				CT_RESULT = CT_FIND_RESULTS["sources"][0]
				TARGET_CT_ID = CT_RESULT["s3aws"]["id"]
				print ("Found CloudTrail Log Source with ID : " + TARGET_CT_ID)
			else:
				print ("No Cloudtrail Log Source found, skipping delete \n")
				EXIT_CODE=0
				sys.exit(EXIT_CODE)
		...
		print ("\n### Starting script - " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M")) + " - Deployment Mode = FIND ###\n")
		APIKEY = args.key
		TARGET_CID = args.cid
		TARGET_CT_NAME = args.ct
		TARGET_DEFENDER = args.dc
		...
		if ALERT_LOGIC_LM != False:
			print ("### Check for existing Cloud Trail Link ###")
			print ("Search for CloudTrail with name =" + TARGET_CT_NAME)
			CT_FIND_RESULTS = find_source_cloudtrail(APIKEY, ALERT_LOGIC_LM, TARGET_CT_NAME, TARGET_CID)
			if CT_FIND_RESULTS != "n/a" and CT_FIND_RESULTS["total_count"] > 0:
				print ("CloudTrail Log Source found:\n" +str(json.dumps(CT_FIND_RESULTS, indent=2)) )
			else:
				print ("No Cloudtrail Log Source found \n")
		else:
			EXIT_CODE=2
			print ("Invalid data center assignment, use -h for more details, stopping ...")
